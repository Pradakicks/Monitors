...
const solveCaptcha = require("../utils/captcha/index");
...
class Footsites {
   ...
   //   consoleIt(res)
    ...
    //  consoleIt(error)
      if (error.response?.status == 503) {
        ...
        consoleIt("Proxy Error Retrying");
   //     consoleIt(error);
        await this.setProxy()
  ...
  async getItemDetails() {
    ...
    //  consoleIt(url);
      ...
      if (error.statusCode == 400) {
        ...
        consoleIt("unable to Initiallized sessions");
        consoleIt("Proxy Error Retrying");
        await this.setProxy()
     //   consoleIt(error.message)
        await this.delay(this.restockDelay);
        await this.getItemDetails();
      }
    }
  ...
  //    consoleIt(this.jar)
   ...
   //   consoleIt(this.jar)
     ...
     //   consoleIt(`Carting product with code: ${this.productInfo.variant}`);
        ...
        if (foundCookie) {
          ...
          let myUrl = JSON.parse(e.error).url;
          let initialURL = myUrl + "&cid=" + foundCookie;
          let recaptchaToken = await solveCaptcha({
            sitekey: '6LccSjEUAAAAANCPhaM2c-WiRxCZ5CzsjR_vd8uX',
            initialURL,
            captchakey : '6a963ac23477cfe46814b0d523b6d395'
          })
          if (!recaptchaToken) {
  ...
  async submitEmail() {
    ...
    try {
      ...
      const res = await this.request(options);
      if (res.status != 200) {
        consoleIt("unable to send email");
      } else {
       // consoleIt(res);
      }
    } catch (e) {
    //  consoleIt(e)
     ...
     //   consoleIt(e)
      ...
      //  consoleIt(e)
        ...
        if (foundCookie) {
          this.ddCookieRes = foundCookie;
          consoleIt("Waiting for datadome");
  ...
  async submitShipping() {
    ...
    try {
      ...
      // consoleIt(res)
      if (res.status != 201) {
        consoleIt("unable to send shipping");
       // consoleIt(res)
        // throw new Error(res)
      } else {
     //   consoleIt(res);
