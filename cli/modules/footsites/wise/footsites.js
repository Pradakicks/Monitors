...
const monitorFootSitesForQueue = () => {
    isWatchingForQueue = true;
    return new Promise((res) => {
        let myInterval = setInterval(async () => {
            try {
                const res = await rp('https://vibris-e98ff-default-rtdb.firebaseio.com/.json', {
                ...
                QueueStatus.footlocker = res.data.footlocker;
                QueueStatus.footaction = res.data.footaction;
                QueueStatus.champssports = res.data.champssports;
                QueueStatus.eastbay = res.data.eastbay;
            } catch (error) {
                // console.log(error)
            }
            if (QueueStatus.footaction && QueueStatus.footlocker && QueueStatus.eastbay && QueueStatus.champssports) {
...
class Footsites {
    ...
    async start() {
        ...
        try {
            ...
            await this.setProxy();
            if (!isWatchingForQueue) {
                monitorFootSitesForQueue()
            }
            ...
            consoleIt("Waiting for Queue")
            await this.checkForWhenQueueIsLive()
    ...
    async checkForWhenQueueIsLive() {
        return new Promise((res) => {
            let myInterval = setInterval(async () => {
                if (QueueStatus[this.site]) {
                    clearInterval(myInterval)
                    res()
                }
            }, 1000)
    ...
    async initializeSession() {
        ...
        let myClientIp = null;
        if (WaitingRoomBank.length > 0) {
            let foundData=WaitingRoomBank[Math.floor(Math.random()*WaitingRoomBank.length)]
            let cookie = await this.request.cookie(foundData.cookie);
            let url = `https://www.${this.site}.com`;
            this.jar.setCookie(cookie, url);
            myClientIp = foundData.ip;
        }
        ...
        try {
            ...
            const options = {
                ...
                headers: {
                    ...
                    "auth-user-id": myClientIp,
                    "true-client-ip": myClientIp
                },
                jar: this.jar,
                proxy: this.proxy,
                rejectUnauthorized: false,
            ...
            if (response.data) {
                ...
                await this.monitorWaitingRoomCookies()
            } else {
    ...
    async monitorWaitingRoomCookies() {
        // console.log(this.jar)
        const myCookies = await this.jar.getCookies(`http://www.${this.site}.com`)
        myCookies.forEach(async (i) => {
            // console.log(i)
            let myCookie = i.toString().split(';')[0].split('Cookie="')[0];
            let cookieNameAndValue = myCookie.split('=')
            let mycookiename = cookieNameAndValue[0]
            let cookieBase64 = cookieNameAndValue[1]
            if (mycookiename.toString().toUpperCase() == "WAITING_ROOM") {
                let myValue = Buffer.from(cookieBase64, 'base64').toString();
                if (myValue.includes("wait")) {
                    // console.log('waiting in queue')
                    if (this.waiting_room) {
                        let cookie = this.request.cookie(`${this.waiting_room}`);
                        let url = `https://www.${this.site}.com`;
                        this.jar.setCookie(cookie, url);
                        // console.log(this.jar)
                    } else {
                        this.waiting_room = myCookie;
                    }
                } else if (myValue.includes("allow")) {
                    // console.log('passed queue');
                    WaitingRoomBank.push({
                        ip:myValue.split('uid=')[1].split('&')[0],
                        cookie:myCookie
                    })
                    return
                }
    ...
    async getDatadomeUrl(body) {
        try {
            let myData = await JSON.parse(body).url;
            if (myData) {
                return myData
            } else {
            ...
            try {
                if (body.includes("Please enable JS and disable any ad blocker")) {
                    const $ = await cheerio.load(body);
                    let scriptTag = await $('script').html()
                    let formattString = await scriptTag.split('dd=')[1].replace(/'/g, `"`);
                    let parseJson = JSON.parse(formattString)
                    delete parseJson.host;
                    delete parseJson.cid;
                    let queryString = Object.keys(parseJson).map(key => key + '=' + parseJson[key]).join('&');
                    let myUrl = `https://geo.captcha-delivery.com/captcha/?initialCid=0525=&${queryString.replace('hsh', 'hash')}`;
                    return myUrl;
                }
            } catch (error) {
                return null
