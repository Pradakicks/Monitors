...
var Collection = helper.ConnectDBShopifyLink()

func NewMonitor(link string, collection *mongo.Collection) *Monitor {
	defer func() {
		if r := recover(); r != nil {
	...
	go func() {
		for !m.stop {
			go func() {
				cur, err := collection.Find(context.TODO(), bson.M{"link": link})
...
func (m *Monitor) monitor() error {
	...
	default:
		...
		url = fmt.Sprintf("https://%s/products.json?limit=%d", m.Config.sku, t)
	}
	// m.BaseUrl = baseUrl
	...
	for _, value := range jsonResponse.Products {
		...
		value.Link = m.Config.sku

		for _, v := range m.products {
			if v.Handle == value.Handle && v.Link == value.Link {
				isPresent = true
				newList = append(newList, ProductsItem{ID: value.ID, UpdatedTime: value.UpdatedAt, Variants: value.Variants, Link: value.Link, Handle: value.Handle})
				if v.UpdatedTime != value.UpdatedAt { // This monitor all changes in a product
					...
					go func(value Types.ShopifyNewProduct) {
						go func(value Types.ShopifyNewProduct) {
							filter := bson.M{"id": value.ID, "handle": value.Handle, "link": value.Link}
							...
							if replacedResult != nil {
								...
								fmt.Println("Remove Result", testing.ID, testing.Link, testing.Handle, replacedResult)
								var testingAgaain Types.ShopifyNewProduct
								isPresent := m.collection.FindOne(context.TODO(), filter).Decode(&testingAgaain)
								fmt.Println("Finding", value.Handle, value.Link, "After", testingAgaain, isPresent)

								result, err := m.collection.InsertOne(context.TODO(), value)
								fmt.Println("Insterting", value.Handle, value.Link, "Now")
								if err != nil {
		...
		if !isPresent && containsKeywords && !containsNegativeKeywords {
			...
			go func(value Types.ShopifyNewProduct) {
				...
				if err != nil {
					...
					fmt.Println("New Product ", value.ID, value.UpdatedAt, value.Link, value.Handle, len(value.Images), len(value.Variants))
					fmt.Println(result)
					link := fmt.Sprintf("https://www.%s.com/products/%s", m.Config.sku, value.Handle)
					var price string
					var image string
