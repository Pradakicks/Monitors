...
func (m *Monitor) monitor() error {
	...
	defer func() {
		watch.Stop()
		fmt.Printf("New Egg Monitor - Status Code : %d : %s %s %t Milliseconds elapsed: %v \n", res.StatusCode, m.Config.sku, m.Config.skuName, m.Availability, watch.Milliseconds())
	}()
...
func webHookSend(c Company, site string, sku string, name string, price int, stockNumber int, time string, image string) {
	payload := strings.NewReader(fmt.Sprintf(`{
		...
		"avatar_url": "%s"
	  }`, site, sku, c.Color, name, price, sku, stockNumber, sku, time, image, c.CompanyImage))
...
func Newegg(w http.ResponseWriter, r *http.Request) {
	...
	go NewMonitor(currentMonitor.Sku, currentMonitor.SkuName, currentMonitor.PriceRangeMin, currentMonitor.PriceRangeMax)
